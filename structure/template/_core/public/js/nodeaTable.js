let NodeaTable = (function () {
	...
	function diveObj(obj, keys, idx = 0) {
		...
		if (Array.isArray(obj[keys[idx]]) && obj[keys[idx]].length > 0) return diveObj(obj[keys[idx]][0], keys, ++idx);
		else if (typeof obj[keys[idx]] === "object") return diveObj(obj[keys[idx]], keys, ++idx);
		else if (obj[keys[idx]] && typeof obj[keys[idx]] !== undefined) return obj;
		else return "-";
	}
	...
	function generateColumnSelector(tableID) {
		...
		for (var column of columns) {
			...
			(function (currentColumn) {
				...
				if (!checked) columnsToHide.push(currentColumn.data);
				var columnDiv = $(
					`<div class='col-6'><label style='cursor:pointer;'><input class="form-control input" name="${
						currentColumn.data
					}" type="checkbox" data-col="${currentColumn.data}" ${checked ? "checked" : ""}>&nbsp;${element.text()}</label></div>`
				);
				// Initialize column checkbox
				var checkbox = columnDiv
				...
				checkbox.on("ifChecked", function () {
					if (columnsToHide.includes(currentColumn.data)) columnsToHide.splice(columnsToHide.indexOf(currentColumn.data), 1);
				});
				...
				checkbox.on("ifUnchecked", function () {
					...
					if (!columnsToHide.includes(currentColumn.data)) columnsToHide.push(currentColumn.data);
				});
		...
		let toggleBtn,
			toggleState = "check",
			halfColumnsNumber = columns.length / 2;
		...
		toggleBtn.on("click", function () {
			...
			$("#columnSelector")
				...
				.each(function (idx) {
					...
					if (idx != 0) {
						if (check) $(this).icheck("checked");
						else $(this).icheck("unchecked");
					}
				});
		});
	...
	let defaults = {
		stateSaveCallback: (tableID, settings, data) => {
			...
			for (var i = 0; i < settings.aoColumns.length; i++) {
				...
				if (size != 0) allZero = false;
				sizes.push(size + "px");
			}
			if (!allZero) localStorage.setItem(tableID + "_columns_sizes", JSON.stringify(sizes));
		},
		stateLoadCallback: (tableID, settings) => {
			var sizes = JSON.parse(localStorage.getItem(tableID + "_columns_sizes"));
			if (!sizes) return;
			var allWidthZero = true;
			for (var i = 0; i < sizes.length; i++) if (sizes[i] != "0px") allWidthZero = false;
			if (allWidthZero) return;
			for (var i = 0; i < settings.aoColumns.length; i++) if (sizes[i]) $(settings.aoColumns[i].nTh).width(sizes[i]);
		},
		saveFilterVal: (tableId, field, value) => {
			var filterSave = JSON.parse(localStorage.getItem("nodea_filter_save_" + tableId));
			if (filterSave == null) filterSave = {};
		...
		getFilterVal: (tableId, field) => {
			...
			try {
				...
				if (filterSave != null && typeof filterSave[field] !== "undefined") val = filterSave[field];
			} catch (err) {
		...
		columns: {
			...
			time: {
				render: ({ value, row, column, entity, additionalData }) => {
					if (value && value.length == 8) return value.substring(0, value.length - 3);
					return value;
				},
				search: ({ column, title, savedFilter, searchTh, triggerSearch, additionalData }) => {
					const element = $(`<input type="text" class="form-control input" value="${savedFilter}" placeholder="${title}" />`);
				...
				render: ({ value, row, column, entity, additionalData }) => {
					...
					if (statusObj.f_name)
						return (
							...
							statusObj.f_color +
							"; color: " +
							statusObj.f_text_color +
							';">' +
							statusObj.f_name +
							"</span>"
						);
					else return '<span class="badge">' + statusObj + "</span>";
				},
			},
			currency: {
				render: ({ value, row, column, entity, additionalData }) => {
					if (value === undefined || value === null) value = "";
					else if (typeof value === "string" && value.indexOf(".") != -1 && value.split(".")[1].length == 1) value += "0";
			...
			file: {
				render: ({ value, row, column, entity, additionalData }) => {
					if (value != "" && value != null)
						return (
							'<a class="file" style="white-space: nowrap;" href="#" data-entity="' +
							entity +
							'" data-value="' +
							value +
							'" data-name="' +
				...
				binding: ({ column, columnDef, entity, element, event, additionalData }) => {
					...
					else if (column.type == "file") element = $(element).find("a");
					else if (column.type == "picture") element = $(element).find("img");
					...
					$.ajax({
						...
						success: function (result) {
							...
							if (result.file.substring(result.file.length, result.file.length - 4).toLowerCase() == ".pdf") {
								...
								fileDisplay =
									"<iframe src=/js/plugins/pdf/web/viewer.html?file=" +
									encodeURIComponent(binaryPDF) +
									' style="width:100%;min-height:500px !important;" allowfullscreen webkitallowfullscreen ></iframe>';
							} else fileDisplay = '<p><img class="img-fluid" src=data:image/;base64,' + result.data + " alt=" + result.file + "/></p>";
							doModal(
								result.file,
			...
			url: {
				render: ({ value, row, column, entity, additionalData }) => {
					if (value != null) return '<a target="_blank" href="' + value + '">' + value + "</a>";
					return value;
				},
			},
			password: {
			...
			text: {
				render: ({ value, row, column, entity, additionalData }) => {
					if (value) {
						...
						var shortText = $.parseHTML(decoded.slice(0, 75))[0].data
							? $.parseHTML(decoded.slice(0, 75))[0].data
							: $(decoded).text().slice(0, 75);
						shortText = HtmlEncode(shortText);
						return (
							"<span style='cursor: pointer;' class='np_text_modal'>" +
							shortText +
				...
				binding: (data) => {
					...
					if (fullText && fullText.length) doModal("Contenu", HtmlDecode($(data.element).find(".full-text").html()));
					else {
						if (defaults && defaults.bindings) defaults.bindings.default(data);
					}
				},
			},
			show: {
		...
		buttons: [
			{
				...
				action: (event, datatable, button, config) => {
					...
					if ($("#columnSelector").length > 0) button.next().remove();
					else button.after(generateColumnSelector("#" + tableID));
				},
			},
			{
				extend: "print",
		...
		dataTableOptions: {
			...
			fnCreatedRow: function (nRow, aData, iDataIndex) {
				if (aData.id) $(nRow).attr("data-id", aData.id);
			},
		},
	};
	...
	function initDataTable(tableID, params = {}, additionalData = {}) {
		if (!tableID) throw new Error("No tableID provided");

		if (params.debug) console.log("tableID", tableID);
		...
		function executeFilter(colIdx, value = "", type = "string", force = false) {
			if (!this.delay) this.delay = 300;

			if (this.timeout)
				if (force) this.delay += 300;
		...
		$(tableID + " .main th", context).each(function (idx) {
			...
			column.show = column.hidden === true ? false : !hiddenColumns || !hiddenColumns.includes(column.data);
			...
			if (params.debug) console.log(column);
		...
		try {
			...
			var tableOptions = {
				...
				ajax: {
					...
					data: function (e) {
						...
						if (tableData[tableID]) Object.assign(e, tableData[tableID] || null);
			...
			if (params.debug) console.log("tableOptions", tableOptions);
			...
			$(tableID + " tbody", context).on("click", "td", function (event) {
				...
				if ($(this)[0] && $(this)[0]._DT_CellIndex && $(this)[0]._DT_CellIndex.column) columnIndex = $(this)[0]._DT_CellIndex.column;

				var column = columns[columnIndex],
					columnDef = columnDefs[columnIndex];

				if (columnDef.binding === false) return true;

				if (!columnDef.binding) {
					console.error(
						"Missing binding function on column definition. Please check if given index match with columnDefs array index. Potential source of the issue: hidden column."
					);
					return true;
				}
